...
//	server grant-role <email> <role>  bootstrap: assign a role (e.g. "admin")
//
...
// a stolen bearer token. grant-role is the friendlier bootstrap for the ordinary
// case: standing up your first "admin" once you have registered an account.
package main
...
// runGrantRole handles `server grant-role <email> <role-key>`.
//
// This is the bootstrap path for the ordinary case: standing up your first
// "admin" once you have registered an account, without reaching for the full
// superuser escape hatch. It bypasses the escalation guard the HTTP role editor
// enforces -- deliberately, since the CLI already requires database access and
// there is no logged-in "actor" to check that guard against.
func runGrantRole(args []string) error {
	if len(args) != 2 || args[0] == "" || args[1] == "" {
		return errors.New("expected exactly two arguments: an email address and a role key")
	}
	email, roleKey := args[0], args[1]
	...
	user, role, err := svc.GrantRoleByKey(ctx, email, roleKey)
	if err != nil {
		if errors.Is(err, identity.ErrNotFound) {
			return fmt.Errorf("no user with email %q, or no role with key %q", email, roleKey)
		}
		return err
	}

	fmt.Printf("granted %q to %s (%s)\n", role.Key, user.Email, user.ID)
	return nil
}
...
//
// It does NOT purge the audit log. That is `server purge`, a separate command
// you schedule yourself, because destroying history should be a deliberate act
// rather than something a long-running process does on a ticker.
//
...
// in the baseline migration, which guards against mistakes, not against an adversary).
...
//
// It honours AUDIT_RETENTION, which defaults to 0 -- "keep forever" -- so with no
// configuration this command does nothing at all.
func runPurge() error {
	cfg, err := settings.Load()
	if err != nil {
		return err
	...
	if cfg.Audit.Retention <= 0 {
		log.Info("nothing to purge: AUDIT_RETENTION is 0 (keep forever)")
		return nil
	}

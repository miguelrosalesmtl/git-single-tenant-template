...
func Invitation(baseURL, inviterEmail, token string) Message {
	link := fmt.Sprintf("%s/invitations/accept?token=%s", baseURL, token)

	return Message{
		Subject: "You have been invited to create an account",
		Body: fmt.Sprintf(`%s has invited you to create an account.
...
//
// Keep the wording plain and the link obvious -- resist the urge to make it
// pretty. Deliverability beats design for an email your own product code may
// gate something important on.

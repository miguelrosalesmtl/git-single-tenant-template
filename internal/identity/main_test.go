...
// `TRUNCATE users CASCADE` would empty `roles` too, taking the seeded SYSTEM
// roles and, through them, the permission catalog's grants with it. Every
// subsequent test would then fail to find the admin role.
//
// So: delete the user-owned data, and leave the catalog and the system roles
// (is_system = true) exactly as the migration seeded them.
func newTestService(t *testing.T) *Service {
	...
	err := database.InTx(ctx, testPool, func(db database.DB) error {
		...
		// Order matters: user_roles.role_id is ON DELETE RESTRICT, so the
		...
		for _, stmt := range []string{
			...
			`DELETE FROM roles WHERE NOT is_system`, // custom roles only; keep the system ones
			`DELETE FROM users`,
		} {
...
// accessFor resolves a user's authority, exactly as the HTTP middleware does.
// Every RBAC rule takes an Access, so the tests build one the same way
// production does rather than fabricating it.
func accessFor(t *testing.T, svc *Service, user User) Access {
	t.Helper()

	access, err := svc.Authorize(context.Background(), user)
	if err != nil {
		t.Fatalf("authorize %s: %v", user.Email, err)
	}
	return access
}

// systemRoleID returns the id of a system role by key ("admin" or "member").
func systemRoleID(t *testing.T, svc *Service, key string) uuid.UUID {
	t.Helper()

	role, err := svc.repo.GetRoleByKey(context.Background(), key)
	if err != nil {

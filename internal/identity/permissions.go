...
// guard something with it). What an admin configures at runtime is which
// permissions each role bundles -- see roles_service.go.
...
// queues.update / queues.delete -- a developer never invents a verb, and an
// admin building a role never has to guess what a word like "manage" covers.
//
// Note what is NOT here: users.create. An account is created by registering or
// by accepting an invitation, never by an admin conjuring one directly, so there
// is no permission gating it -- those routes are guarded by rate limiting alone.
const (
	// Users. There is no users.create; see above. users.update covers both
	// changing which roles a user holds and activating/deactivating them.
	...
	// together -- so you can grant "may edit roles but not delete them".
	...
	// API keys. Managing them is an administrative act, so it is split
	// create/read/delete like roles.
...
var Catalog = []CatalogEntry{
	{PermUsersRead, "View the users in this installation"},
	{PermUsersUpdate, "Change which roles a user holds, and activate or deactivate them"},

	{PermInvitationsRead, "View pending invitations"},
	{PermInvitationsCreate, "Invite people to create an account"},
	{PermInvitationsDelete, "Revoke a pending invitation"},

	{PermRolesRead, "View the roles this installation uses"},
...
// PermissionSet is what a caller may do: the union of the permissions granted by
// every role they hold.
//
// It is a set, not a ladder. A ladder can only ask "are you at least an admin?",
// which cannot express "may manage billing but not users", because authority
// would be one-dimensional. A set has no ordering, and every check is simply "is
// this permission in it?".
...
// permissions, or assigns a role to a user, the service checks that the actor's
// own permission set is a superset of the permissions being handed out.
//
// Without it, RBAC is self-defeating: a member holding roles.create would simply
// create a role carrying users.update, assign it to themselves, and have escaped
// every limit you placed on them. The same one rule also stops them assigning the
// system "admin" role, since admin carries permissions they do not hold -- no

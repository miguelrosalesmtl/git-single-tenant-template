...
// handleListRoles returns every role this installation uses: the two system
// roles plus any custom ones an admin created.
...
// handleCreateRole builds a custom role.
//
// Requires roles.create AND -- enforced in the service -- that the caller
// already holds every permission they are putting into the new role. Otherwise
// roles.create would just be a long-winded way of spelling "admin".
...
// handleSetUserRoles replaces the set of roles a user holds.
//
...
// accidentally apply a change twice. An empty list is valid: it leaves the user
...
func (s *Server) handleSetUserRoles(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	if err := s.identity.SetUserRoles(ctx, userFrom(ctx), accessFrom(ctx), targetID, req.RoleIDs); err != nil {
		s.errors.handle(w, r, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)

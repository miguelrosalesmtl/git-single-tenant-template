...
// service tests: the middleware chain (auth, permission resolution, the
// superuser bypass), status-code mapping, security headers, CORS, the rate
// limiter, and request decoding.
//
...
// cleanDatabase empties the user-owned tables, leaving the seeded system roles
// and the permission catalog. It mirrors internal/identity's cleanup, including
// the audit-purge dance the append-only trigger forces even on tests.
...
// testAuthSettings turns argon2 down to the cheapest thing that still works. See
// internal/identity for the rationale.
...
// makeAdmin grants the system admin role directly, the way the CLI bootstrap
// command does, so tests can get an administrator without going through HTTP.
func (h *harness) makeAdmin(email string) {
	h.t.Helper()
	if _, _, err := h.svc.GrantRoleByKey(context.Background(), email, identity.RoleKeyAdmin); err != nil {
		h.t.Fatalf("grant admin to %s: %v", email, err)
	}
}

// roleID returns the id of a role by key, as visible via the API.
func (h *harness) roleID(token, key string) uuid.UUID {
	h.t.Helper()
	rec := h.req(http.MethodGet, "/api/v1/roles", token, nil)
	mustStatus(h.t, rec, http.StatusOK)
	var resp struct {
		Roles []struct {
			ID  uuid.UUID `json:"id"`
	...
	h.t.Fatalf("no role with key %q", key)
	return uuid.Nil
}

...
func (h errorHandler) handle(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, identity.ErrLastAdmin):
		writeError(w, http.StatusConflict, "this installation must always have at least one admin")
	...
	case errors.Is(err, identity.ErrSystemRole):
		writeError(w, http.StatusForbidden,
			"the admin and member roles are built in and cannot be changed -- create a custom role instead")
	...
	case errors.Is(err, identity.ErrRoleKeyTaken):
		writeError(w, http.StatusConflict, "a role with that key already exists")
...
func (h errorHandler) auditDenial(r *http.Request, err error) {
	...
	// The request may have failed before authentication was established, so that
	// is not assumed either. An unattributed denial is still worth recording --
	// often more so.

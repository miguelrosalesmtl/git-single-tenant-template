// Package audit records an append-only history of who did what, from where, and
// in which request.
//
// Nothing in this package updates or deletes, and a database trigger (see the
// baseline migration) refuses both if anything tries.
//
...
const (
	...
	ActionUserRolesUpdated       Action = "users.roles_updated"

	// --- Invitations ---
	ActionInvitationCreated  Action = "invitations.created"
	ActionInvitationRevoked  Action = "invitations.revoked"
	ActionInvitationAccepted Action = "invitations.accepted" // the account was created
	...
	// --- Superuser ---
	// Granting or revoking the one global privilege that outranks RBAC entirely.
	// Both are CLI-only; the actor is NULL because there is no logged-in user
	// behind a shell command.
	ActionSuperuserGranted Action = "superuser.granted"
	...
	// ActionAccessDenied is a 403: the caller was authenticated but lacked the
	// permission the route required. A burst of these from one user is somebody
	// mapping your authorization boundaries.
...
//
// ActorUserID is a pointer because plenty of events have none: a failed login has
...
func (r *Recorder) Record(ctx context.Context, e Event) error {
	...
	_, err = r.db.Exec(ctx,
		...
		e.ActorUserID, e.Action, e.TargetType, e.TargetID, raw,
...
// Filter narrows a listing. The zero value matches everything.
type Filter struct {
...
func (r *Recorder) List(ctx context.Context, f Filter) ([]Entry, error) {
	limit := f.Limit
	...
	rows, err := r.db.Query(ctx,
		`SELECT id, actor_user_id, action, target_type, target_id, metadata,
		 ...
		 ORDER BY id DESC
		 LIMIT $6`,
		before, action, actor, from, to, limit,
	)
	if err != nil {
...
func (r *Recorder) Purge(ctx context.Context, retain time.Duration) (int64, error) {
	...
	// The trigger rejects every DELETE unless this is set. It is transaction-local
	// (SET LOCAL), so it cannot leak into another statement on a pooled connection.
